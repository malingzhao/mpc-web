...
public class DistributedMPCClient {
    ...
    public boolean startKeygen() {
        try {
            ...
            sessionData.addProperty("session_type", "keygen");
            sessionData.addProperty("party_id", partyId);
            sessionData.addProperty("threshold", threshold);
            sessionData.addProperty("total_parties", totalParties);
            ...
            request.addProperty("data", gson.toJson(sessionData));
    ...
    private void handleSessionCreated(JsonObject message) {
        ...
        // 初始化密钥生成但不立即开始
        try {
            keygenHandle = MPCNative.keygenInit(1, partyId, threshold, totalParties); // 0 = SECP256K1
            if (keygenHandle == 0) {
    ...
    private void executeRound3() {
        ...
        if (dkgKey == null) {
            ...
            // 将密钥数据保存到文件
            try {
                String keyFileName = "test_key_party" + partyId + ".key";
                String keyDataJson = java.util.Base64.getEncoder().encodeToString(dkgKey);
                java.nio.file.Files.write(java.nio.file.Paths.get(keyFileName), keyDataJson.getBytes());
                System.out.println("💾 Java客户端 " + clientId + " 密钥已保存到文件: " + keyFileName);
            } catch (Exception e) {
                System.err.println("❌ Java客户端 " + clientId + " 保存密钥文件失败: " + e.getMessage());
            }
